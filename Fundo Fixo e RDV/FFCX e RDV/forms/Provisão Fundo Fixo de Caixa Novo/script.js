                            ...
                            <div class="row">
                                <div class="col-md-6 col-lg-6">
                                    <label class="labelFullWidth" style="width: 100%">Produto:
                                        <input name="ProdutoItem" type="text" class="form-control produto" readonly id="produto${ordem}" value="${item.descricao}"/>
                                    </label>
                                    <br>
                                </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6" id="produtosInfo${ordem}">
                                                <label class="labelFullWidth" style="width: 100%">Valor Unitário:
                                                    <input type="text" name="ValorUnitItem" class="form-control ValorUnitItem" id="inputValorUnitEdicao${ordem}" oninput="FormataNumeros(this); atualizarValorTotalFFCX(); atualizarValorTotal(${ordem}); ${FormataValorInserir(this.val)}; AtualizaValorCampoValorUnitEdicao(${ordem})">
                                                </label>
                                                <br>
...
function atualizarValorTotalFFCX() {
  ...
  document.getElementById('valorTotalFFCX').innerText = 'R$ ' + total.toLocaleString('pt-BR', {
...
function preencherLocalEstoque(dsFinalLocEstoque, selectedFilial) {
  ...
  for (var i = 0; i < dsFinalLocEstoque.length; i++) {
    if (selectedFilial == dsFinalLocEstoque[i].CODFILIAL) {
      var optionValue =
        dsFinalLocEstoque[i].CODCCUSTO + " - " + dsFinalLocEstoque[i].NOME;
      $("#selectLocalEstoque").append(
  ...
  if (formMode !== "VIEW") {
    link.addEventListener("click", function (event) {
      ...
      for (var campoId in campos) {
        var valorCampo = $("#" + campoId).val().trim();
          ...
          tabela.DataTable({
            columns: [
              ...
              },
              {
                ...
                render: function (data, type, row) {
                  return (
                    "<input class='Idmov1207' readonly style='border: 0' value='" + data + "'>"
                  );
                },
              },
              {
              ...
              },
              {
                ...
                render: function (data, type, row) {
                  return (
                    "<input class='ValorMov1207' readonly style='border: 0;' readonly value='" + FormataValor(data) + "'>"
                  );
                },
              },
            ],
  ...
  return new Promise((resolve, reject) => {
    DatasetFactory.getDataset(
      ...
      [
        ...
        DatasetFactory.createConstraint("permissaoGeral", VerificaSeUsuarioPermissaoGeral() ? "true" : "false", VerificaSeUsuarioPermissaoGeral() ? "true" : "false", ConstraintType.MUST)
      ],
      null,
      {
...
$('ul.nav.nav-tabs.nav-justified.nav-pills a[data-toggle="tab"]').on(
  "shown.bs.tab",
  function (e) {
...
$("#modalidade").on("change", function () {
  ...
  if (activity == 7) {
    ...
    puxaFormaPgto();
    if (firstOption) {
      $("#fundoFixo option").filter(function () {
...
function AddItem() {
  ...
  if ($("#divbtnAddItens").is(":hidden")) {
    $("#tabItens, #divbtnAddItens, .itens, [id^=divItensProdutos], [id^=divCorpoTabela], #icon-green, #divValorTotal, #titleValorTotal, #valorTotalFFCX").show();
    atualizarValorTotalFFCX();
    exibirItensArmazenados();
    $("#divbtnVoltarAosItens, #painelTabelaDeProdutos").hide();
  } else {
    $("#mensagemNenhumaItem, #faturamentoDecisao, #icon-green, #divbtnAddItens, .itens, [id^=divItensProdutos], [id^=divCorpoTabela], #divValorTotal, #titleValorTotal, #valorTotalFFCX").hide();
...
function mostrarDetalhes(IDMOV) {
  ...
  for (total = 0; total < DatasetShowItems.values.length; total++) {
    ...
    if ($(".CheckFatItem").val() == "notChecked") {
      retorno +=
        '<input type="checkbox" id="CheckFatItem" name="CheckFatItem" class="CheckFatItem"/>';
    } else {
      retorno +=
        '<input type="checkbox" id="CheckFatItem" name="CheckFatItem" class="CheckFatItem" checked/>';
    }
    retorno +=
    ...
    retorno +=
      ...
      <h4 class="panel-title">\
             <a class="collapse-icon collapsed" data-toggle="collapse" data-parent="#accordion" href="#' + DatasetShowItems.values[total].NSEQITMMOV + '" >\
               <div class="row">\
               <div id="divFornecedor" class="col-md-12">\
                 <span style="font-weight: bold">Fornecedor: </span>\
                 <input class="Fornecedor1207" readonly="" \
                   value="' + DatasetShowItems.values[total].HISTORICOCURTO + '"\
             ...
             <div class="row" style="margin-top: 12px">\
               <div id="divProduto" class="col-md-9">\
                 ...
                 <input\
                   ...
                   value="' + DatasetShowItems.values[total].NOMEFANTASIA + "-" + DatasetShowItems.values[total].CODIGOPRD + '"\
                 />\
               </div>\
               <div id="divValor" class="col-md-3">\
                 ...
                 <input\
                   ...
                   value="' + FormataValor(DatasetShowItems.values[total].PRECOUNITARIO) + '"\
                 />\
               </div>\
             </div>\
                </a\
    ...
    for (h = 0; h < datasetshowRateiosCentrodeCusto.values.length; h++) {
      if (
        DatasetShowItems.values[total].NSEQITMMOV ==
        datasetshowRateiosCentrodeCusto.values[h].NSEQITMMOV
      ) {
        retorno +=
          ' <div class="row">\
    ...
    for (var k = 0; k < datasetshowRateiosDepatamento.values.length; k++) {
      if (
        DatasetShowItems.values[total].NSEQITMMOV ==
        datasetshowRateiosDepatamento.values[k].NSEQITMMOV
      ) {
        retorno +=
          '<div class="row"\
              ...
              <div class="panel-body divCorpoTabela" id="divCorpoTabela${ordem}">
                  <div style="width: 100%;">
                      <div class="row">
                          <div class="col-md-6 col-lg-6">
                              <label class="labelFullWidth" style="width: 100%">Produto:
                                  <input name="ProdutoItem" readonly disabled id="produto${ordem}" type="text" class="form-control produto" value="${dsFinalItems[j].NOMEFANTASIA}" />
                          ...
                          <div class="col-md-6 col-lg-6">
                              <div class="row">
                                  ...
                                  <div class="col-md-6">
                                      <label class="labelFullWidth" style="width: 100%">Valor Unitário:
                                          <input type="text" readonly disabled name="ValorUnitItem" class="form-control ValorUnitItem" id="inputValorUnit${ordem}" oninput="atualizarValorTotal(${ordem})" placeholder="R$ 0,0000" value="${FormataValorInserir(valorItem)}">
                                      </label>
                                      <br>
                                  </div>
                              </div>
                      ...
                      <div class="col-md-12">
                          <label class="labelFullWidth" style="width: 100%; margin-top: 10px">Prestador/Fornecedor:
                              <textarea name="fornecedor" readonly disabled class="form-control fornecedor" id="fornecedor${ordem}" oninput="AlteraDescricao(this, ${ordem})">${dsFinalItems[j].HISTORICOCURTO}</textarea>
                          </label>
                          <br>
                      </div>
                      <div class="col-md-12">
                          <label class="labelFullWidth" style="width: 100%; margin-top: 10px">Centro de Custo:
                              <select class="form-control codCC" readonly disabled id="selectCentroDeCusto${ordem}" value="${dsFinalRatCC[j].CODCCUSTO} - ${dsFinalRatCC[j].NOMECENTRODECUSTO}">
                                  <option value="${dsFinalRatCC[j].CODCCUSTO} - ${dsFinalRatCC[j].NOMECENTRODECUSTO}" readonly>${dsFinalRatCC[j].CODCCUSTO} - ${dsFinalRatCC[j].NOMECENTRODECUSTO}</option>
                              </select>
                          </label>
                      </div>
                      <div class="col-md-12">
                          <label class="labelFullWidth departamentoDiv" style="width: 100%; margin-top: 10px">Departamento:
                              <select class="form-control departamento" readonly disabled id="departamentoHtml${ordem}" value="${dsFinalRatDep[j].CODDEPARTAMENTO}">
...
function atualizarValorTotal(ordem) {
  ...
  if (quantidadeElement && valorUnitarioElement) {
    ...
    if (valorTotalElement) {
      valorTotalElement.value = valorTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }
  } else {
  ...
  if ($("#formMode").val() == "ADD") {
    ...
    $(".divFatItem").each(function () {
      if ($(this).find(".CheckFatItem").prop("checked")) {
        var json = {
          values:
            DatasetDadosMovAprov.values[$(this).find(".ValNum").val() - 1],
        };
...
function handleAprovContabilidade() {
  if ($("#decisaoAprovar").prop("checked")) {
    ...
    let DataEmailRetorno = new Date();
    let dataFormatadaEmailRetorno = DataEmailRetorno.getDate() + "/" + (DataEmailRetorno.getMonth() + 1) + "/" + DataEmailRetorno.getFullYear();
    $("#DataEmail").val(dataFormatadaEmailRetorno);
  }
}
    ...
    if (atividadeDto == 6) {
      if ($("#aprovacao").val() == "sim") {
        ...
        let dataFormatadaEmail =
          dataEmail.getDate() +
          "/" +
          (dataEmail.getMonth() + 1) +
          "/" +
          dataEmail.getFullYear();
        $("#DataEmail").val(dataFormatadaEmail);
        handleProvisao();
        $("#valuesRecebimento").val(JSON.stringify(listJson1207));
        ...
        tabela.DataTable({
          columns: [
            ...
            },
            {
              ...
              render: function (data, type, row) {
                return (
                  "<input class='Idmov1207' readonly style='border: 0' value='" +
                  data +
                  "'>"
                );
              },
